...
//You can get everything from dot notation with objects, even nested properties:
const persona = {
  nombre: 'Andrea',
  direccion: {
    calle: {
...
// Acceder al número
console.log(persona.direccion.calle.numero); // 123


//An E-commerce example that creates a product object:
const items = [
  {
...
items.forEach((item) => {
  ...
  // Añadir al DOM
  document.querySelector(".card-list__items").append(cardElement); //Selecciona el div vacío 
  //El contenedor vacío donde irán las cards con la clase card-list__items
});

//Now the e-commerce example is optimized to make it more reusable, but that makes it MORE ABSTRACT:
const items = [
  {
    image: "https://practicum-content.s3.us-west-1.amazonaws.com/web-code/oop/moved_card_detail.jpg",
    title: "BIOLOID",
...
class Card {
  constructor(data, cardSelector) { //The parameter data is an object with the properties title, description, price, and image
    this._title = data.title;  
    this._description = data.description;
    this._price = data.price;
    this._image = data.image;
    this._cardSelector = cardSelector; /*  ¿Pero qué es this._cardSelector?
Eso lo definiste en el constructor con:
this._cardSelector = cardSelector;
Y eso significa que ahora puedes pasarle diferentes strings como:
...
'.vertical-card'*/
  ...
  _getTemplate() {
    const cardElement = document
      .querySelector(this._cardSelector)  //This changed
      .content
      .querySelector('.card')
      .cloneNode(true);
...
//Básicamente siento que es mas escalable y reutilizable, 
// pero mas abstracto porque no estoy usando un valor que le corresponda a ningun item, sino a lo que represente el item "de ese momento"
//data (el objeto item) → representa el contenido de una card.
//cardSelector → representa la estructura visual donde ese contenido se va a mostrar.